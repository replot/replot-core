...
const GradientLegend = (gradient, scale, title, isBackgroundDark) => {
  const HEIGHT = 15
  const WIDTH = 30
  const FONT_SIZE = 14
  ...
  legend.push(
    <linearGradient id={`gradient-${gradient[0].color}-${gradient[99].color}`} key="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
  ...
  legend.push(
    <rect key="rect" x={0} y={-HEIGHT/2 + titleHeight}
      height={HEIGHT} width={WIDTH * 10} fill={`url(#gradient-${gradient[0].color}-${gradient[99].color})`}/>
  )
